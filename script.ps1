...
# Set up log file in the repo folder
...
### [TASK 1] Ensure Winget, Choco & NuGet Installed/Updated (Refactored Monolith)
Write-Log "[TASK 1] Ensure Winget, Choco & NuGet Installed/Updated - START" 'INFO'
...
if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
    ...
    try {
        ...
        $wingetStatus.Status = 'Installed'
        $wingetStatus.Method = 'App Installer'
    } catch {
        Write-Log "[Winget] Install failed: $_" 'ERROR'
        $wingetStatus.Status = 'Failed'
        $wingetStatus.Error = $_
    }
} else {
    ...
    try {
        ...
        $wingetStatus.Status = 'Updated'
        $wingetStatus.Method = 'winget upgrade'
    } catch {
        Write-Log "[Winget] Update failed: $_" 'WARN'
        $wingetStatus.Status = 'UpdateFailed'
        $wingetStatus.Error = $_
    }
}
...
if (-not (Get-Command choco -ErrorAction SilentlyContinue)) {
    Write-Log "[Choco] Not found. Attempting to install..." 'WARN'
    try {
        Set-ExecutionPolicy Bypass -Scope Process -Force
        [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072
        Invoke-Expression ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1'))
        Write-Log "[Choco] Installed." 'INFO'
        $chocoStatus.Status = 'Installed'
        $chocoStatus.Method = 'Install Script'
    } catch {
        Write-Log "[Choco] Install failed: $_" 'ERROR'
        $chocoStatus.Status = 'Failed'
        $chocoStatus.Error = $_
    }
} else {
    ...
    try {
        ...
        $chocoStatus.Status = 'Updated'
        $chocoStatus.Method = 'choco upgrade'
    } catch {
        Write-Log "[Choco] Update failed: $_" 'WARN'
        $chocoStatus.Status = 'UpdateFailed'
        $chocoStatus.Error = $_
    }
}
...
if (Get-Command nuget -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        $nugetStatus.Status = 'Updated'
        $nugetStatus.Method = 'WebRequest'
    } catch {
        Write-Log "[NuGet] Update failed: $_" 'ERROR'
        $nugetStatus.Status = 'UpdateFailed'
        $nugetStatus.Error = $_
    }
    ...
    try {
        ...
        $nugetStatus.Status = 'Installed'
        $nugetStatus.Method = 'WebRequest'
    } catch {
        Write-Log "[NuGet] Install failed: $_" 'ERROR'
        $nugetStatus.Status = 'Failed'
        $nugetStatus.Error = $_
    }
}
...
try {
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        Write-Log "[NuGet] Attempting to install NuGet provider via winget (Microsoft.NuGet)..." 'INFO'
        try {
            ...
            Write-Log "[NuGet] Exception during provider install via winget: $_" 'WARN'
        }
    }
    ...
    if (-not $provider -or $provider.Version -lt [version]'2.8.5.201') {
        Write-Log "[NuGet] Installing or updating provider for PowerShell (unattended)..." 'INFO'
    ...
    Write-Log "[NuGet] Failed to install or update provider for PowerShell: $_" 'WARN'
}

# Summary
Write-Log "[TASK 1] Summary: Winget: $($wingetStatus.Status), Choco: $($chocoStatus.Status), NuGet: $($nugetStatus.Status)" 'INFO'
if ($wingetStatus.Error) { Write-Log "[Winget] Error: $($wingetStatus.Error)" 'ERROR' }
if ($chocoStatus.Error) { Write-Log "[Choco] Error: $($chocoStatus.Error)" 'ERROR' }
if ($nugetStatus.Error) { Write-Log "[NuGet] Error: $($nugetStatus.Error)" 'ERROR' }
Write-Log "[TASK 1] Ensure Winget, Choco & NuGet Installed/Updated - END" 'INFO'
