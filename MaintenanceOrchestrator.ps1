...
<#
.SYNOPSIS
    Resolves the project root based on expected folder layout.

.DESCRIPTION
    Checks candidate roots for the required config and modules directories and
    returns the first valid path, falling back to a secondary path when needed.

.PARAMETER PrimaryPath
    Preferred root candidate.

.PARAMETER FallbackPath
    Secondary root candidate when the primary is invalid.
...
$CoreModules = @(
    ...
    'DiffEngine',           # v4.1: Diff list generation
...
foreach ($moduleName in $CoreModules) {
    ...
    try {
        ...
        Write-Information "  Info: Try running: Unblock-File '$modulePath'" -InformationAction Continue
        exit 1
    }
    catch [System.Security.SecurityException] {
        Write-Error "Security error loading module $moduleName. Check execution policy and file permissions."
        Write-Information "  Info: Current execution policy: $(Get-ExecutionPolicy)" -InformationAction Continue
        exit 1
    }
    catch [System.IO.FileNotFoundException] {
        Write-Error "Module file not found: $modulePath"
        Write-Information "  Info: Ensure all module files are present in the modules/core directory" -InformationAction Continue
        exit 1
    }
    catch {
        Write-Error "Failed to load core module $moduleName`: $_"
        Write-Information "  Info: Error Type: $($_.Exception.GetType().Name)" -InformationAction Continue
        Write-Information "  Info: Error Details: $($_.Exception.Message)" -InformationAction Continue
        if ($_.ScriptStackTrace) {
            Write-Information "  Info: Stack Trace: $($_.ScriptStackTrace)" -InformationAction Continue
        }
        exit 1
    }
}
...
try {
    ...
    if ($validationResult.AllValid) {
        Write-Information "   [OK] All $($validationResult.ValidConfigs) configuration files validated successfully" -InformationAction Continue
    }
    else {
        ...
        foreach ($result in $validationResult.Results) {
            if (-not $result.IsValid) {
                Write-Information "   [FAIL] $($result.Name): $($result.Errors -join '; ')" -InformationAction Continue
            }
        }
...
foreach ($moduleName in $Type2Modules) {
    ...
    try {
        ...
        Write-Information "  Info: This module will be skipped during execution" -InformationAction Continue
...
try {
    ...
    if (Get-Command -Name 'Get-RegisteredModules' -ErrorAction SilentlyContinue) {
        ...
        if ($dependencyFailures.Count -gt 0) {
            ...
            Write-Information "   [OK] All module dependencies validated successfully" -InformationAction Continue
        }
    }
    else {
...
Write-Information "`nDetecting Windows Version..." -InformationAction Continue
try {
    # Detect OS version for OS-specific operations
    $osContext = Get-WindowsVersionContext
    Set-Variable -Name 'OSContext' -Scope Global -Value $osContext
...
#>
function Set-SystemRestorePointSpace {
    ...
    try {
        ...
        try {
            $enableRestoreCommand = Get-Command -Name 'Enable-ComputerRestore' -ErrorAction SilentlyContinue
            if ($enableRestoreCommand) {
                if ($PSCmdlet.ShouldProcess($systemDrive, 'Enable System Protection')) {
                    & $enableRestoreCommand -Drive $systemDrive -ErrorAction SilentlyContinue | Out-Null
                }
            }
        }
        catch {
        ...
        try {
            ...
            if ($srp) {
                ...
                if ($currentAllocationGB -lt $MinimumGB) {
                    ...
                    if (-not $PSCmdlet.ShouldProcess($systemDrive, "Set System Restore Point allocation to $MinimumGB GB")) {
                        return $false
                    }
...
try {
    ...
    if ($createRestorePoint) {
        Set-SystemRestorePointSpace -MinimumGB $restorePointMinSizeGB | Out-Null
    }
    ...
    if ($createRestorePoint) {
        ...
        if (-not $restoreResult) {
            try {
                ...
                $checkpointCommand = Get-Command -Name 'Checkpoint-Computer' -ErrorAction Stop
                & $checkpointCommand -Description $restoreDescription -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
                $restoreResult = @{
                    Success = $true
...
if (-not (Get-Command -Name 'Write-LogEntry' -ErrorAction SilentlyContinue)) {
    <#
    .SYNOPSIS
        Fallback logging helper when LoggingManager is unavailable.

    .DESCRIPTION
        Writes basic formatted log messages to the console to preserve visibility
        when structured logging is not loaded.
    ...
    .PARAMETER Data
        Optional structured data (ignored in fallback).
    #>
...
try {
    ...
    Write-Information "  Info: Some operations may have limited functionality" -InformationAction Continue
}
#endregion
...
try {
    try {
        ...
        try {
            ...
            Write-Information "  Info: Continuing with basic validation - some configuration issues may cause runtime errors" -InformationAction Continue
        }
        ...
        try {
            ...
            Write-Information "  Info: Continuing with basic file operations - some features may be limited" -InformationAction Continue
        ...
        try {
            ...
            Write-Information "  Info: Modules will attempt to create directories as needed" -InformationAction Continue
        }
        ...
        try {
            ...
            Write-Information "  Info: Continuing without enhanced logging - basic console output only" -InformationAction Continue
        }
        ...
        Write-Information "  Info: Ensure the 'config' directory exists and contains required configuration files" -InformationAction Continue
        exit 1
    }
    catch [System.IO.FileNotFoundException] {
        Write-Error "Required configuration file not found: $($_.Exception.Message)"
        Write-Information "  Info: Ensure all required configuration files are present in: $ConfigPath" -InformationAction Continue
        exit 1
    }
    catch [System.Management.Automation.RuntimeException] {
        Write-Error "Configuration system error: $($_.Exception.Message)"
        Write-Information "  Info: Check configuration file syntax and module dependencies" -InformationAction Continue
        exit 1
    }
    catch {
        Write-Error "Failed to initialize configuration: $($_.Exception.Message)"
        Write-Information "  Info: Error Type: $($_.Exception.GetType().Name)" -InformationAction Continue
        Write-Information "  Info: This may indicate missing dependencies or corrupted configuration files" -InformationAction Continue
        exit 1
    }
    ...
    try {
        ...
        try {
            ...
            Write-Information "  Info: Bloatware removal tasks will be skipped" -InformationAction Continue
            $BloatwareLists = @()
        }
        try {
            ...
            Write-Information "  Info: Essential app installation tasks will be skipped" -InformationAction Continue
            $EssentialApps = @()
        }
        ...
        Write-Information "  Info: Ensure bloatware-list.json and essential-apps.json exist in: $ConfigPath" -InformationAction Continue
        exit 1
    }
    catch [System.ArgumentException] {
        Write-Error "Invalid app configuration format: $($_.Exception.Message)"
        Write-Information "  Info: Check JSON syntax and structure in app configuration files" -InformationAction Continue
        exit 1
    }
    catch {
        Write-Error "Failed to load app configurations: $($_.Exception.Message)"
        Write-Information "  Info: Error Type: $($_.Exception.GetType().Name)" -InformationAction Continue
        Write-Information "  Info: Check app configuration files in: $ConfigPath" -InformationAction Continue
        exit 1
    }
    #endregion
    ...
    <#
.SYNOPSIS
    Invokes a task function with module-specific parameters.

.DESCRIPTION
    Ensures the target function receives the expected Config parameter or
    task-specific flags based on the module name.
...
.OUTPUTS
    [object] Result from the invoked function.
    #>
    function Invoke-TaskWithParameter {
        param(
            [string]$TaskName,
            [string]$FunctionName,
            [hashtable]$Config
...
.OUTPUTS
    [string] Path to created session.json manifest file.
    #>
    function New-SessionManifest {
        ...
        try {
            if (-not $PSCmdlet.ShouldProcess('Session', 'Create session manifest')) {
                return $null
            }
    ...
    #region ORIGINAL VISION: 5-Stage Execution Flow
    # STAGE 1: Interactive Type1 Audit Selection
    ...
    if (-not $NonInteractive -and -not $TaskNumbers) {
        ...
        if (Test-Path $type1Path) {
            ...
            foreach ($file in $type1Files) {
                # Create module info with descriptions
                $moduleName = $file.BaseName
                $description = switch -Wildcard ($moduleName) {
                    '*Bloatware*' { 'Scan for unwanted pre-installed applications' }
                    '*EssentialApps*' { 'Check for missing essential applications' }
                    '*SystemOptimization*' { 'Analyze system optimization opportunities' }
                    '*Telemetry*' { 'Detect active Windows telemetry services' }
                    '*Security*' { 'Evaluate system security posture' }
                    '*WindowsUpdates*' { 'Check for pending Windows updates' }
                    '*AppUpgrade*' { 'Identify available application upgrades' }
        ...
        if ($selectedType1Indices -contains 0) {
            ...
            $selectedType1Modules = $type1ModulesAvailable
            Write-Host "`n  [OK] Selected: All $($type1ModulesAvailable.Count) Type1 audit modules" -ForegroundColor Green
        }
        else {
            # User selected specific modules
            foreach ($index in $selectedType1Indices) {
                if ($index -ge 1 -and $index -le $type1ModulesAvailable.Count) {
                    $selectedType1Modules += $type1ModulesAvailable[$index - 1]
                }
            }
        ...
        foreach ($module in $selectedType1Modules) {
            ...
            try {
                ...
                if ($functionName) {
                    ...
                    Write-Host " [WARN] No execution function found" -ForegroundColor Yellow
                    Write-LogEntry -Level 'WARNING' -Component 'ORCHESTRATOR-STAGE1' -Message "Type1 module has no execution function: $($module.Name)"
                }
            }
            catch {
                Write-Host " [FAIL]" -ForegroundColor Red
                Write-Warning "  Error: $($_.Exception.Message)"
                Write-LogEntry -Level 'ERROR' -Component 'ORCHESTRATOR-STAGE1' -Message "Type1 audit failed: $($module.Name)" -Data @{ Error = $_.Exception.Message }
        ...
        # Build diff lists from audit results
        $diffResultSet = $null
        if (Get-Command -Name 'New-DiffListSet' -ErrorAction SilentlyContinue) {
            $diffResultSet = New-DiffListSet -MainConfig $MainConfig -AuditResults $AuditResults -OSContext $script:OSContext
            $script:DiffLists = if ($diffResultSet.DiffLists) { $diffResultSet.DiffLists } else { @{} }
            Write-LogEntry -Level 'INFO' -Component 'ORCHESTRATOR-STAGE1' -Message 'Diff lists generated from Type1 audit results'
        }
        else {
            Write-Warning "DiffEngine not available - Type2 selection may be inaccurate"
        }
        ...
        # ============================================================
        Write-Host "============================================================" -ForegroundColor Magenta
        Write-Host "  STAGE 2: ANALYZING AUDIT RESULTS" -ForegroundColor White
        Write-Host "============================================================" -ForegroundColor Magenta
        Write-Host ""
        Write-Host "  Processing audit data to determine required maintenance tasks..." -ForegroundColor Cyan
        Write-Host ""

        # Analyze diff lists to determine which Type2 modules have work to do
        $type2Recommendations = @()
        $diffLists = if ($diffResultSet -and $diffResultSet.DiffLists) { $diffResultSet.DiffLists } else { @{} }

        foreach ($task in $AvailableTasks) {
            $moduleName = if ($task.ModuleName) { $task.ModuleName } else { $task.Name }
            $shouldRun = $false
            $reason = 'No actionable items in diff list'
            $detectedCount = 0

            if ($diffLists.ContainsKey($moduleName)) {
                $diffEntry = $diffLists[$moduleName]
                $detectedCount = if ($diffEntry.DiffCount) { $diffEntry.DiffCount } else { 0 }
            ...
            if ($shouldRun) {
                $type2Recommendations += @{
                    ...
                    Reason = $reason
                    Count = $detectedCount
                    Priority = switch ($moduleName) {
                        'SecurityEnhancement' { 'HIGH' }
                        'BloatwareRemoval' { 'HIGH' }
                        'SystemOptimization' { 'MEDIUM' }
                        'TelemetryDisable' { 'MEDIUM' }
                        'EssentialApps' { 'MEDIUM' }
                        'WindowsUpdates' { 'MEDIUM' }
                        'AppUpgrade' { 'LOW' }
                        default { 'MEDIUM' }
                    }
                }
                ...
                Write-Host "No actions to be taken (diff list empty)" -ForegroundColor Gray
            }
        }
        ...
        if ($type2Recommendations.Count -eq 0) {
            ...
            # ============================================================
            Write-Host "============================================================" -ForegroundColor Magenta
            Write-Host "  STAGE 3: MAINTENANCE ACTIONS (Type2 Modules)" -ForegroundColor White
            Write-Host "============================================================" -ForegroundColor Magenta
            Write-Host ""
            ...
            if ($selectionInput -eq "0" -or [string]::IsNullOrWhiteSpace($selectionInput)) {
                ...
                Write-Host ""
                Write-Host "  [OK] Selected: All $($type2Recommendations.Count) recommended tasks" -ForegroundColor Green
            }
            else {
                # Parse comma-separated numbers
                try {
                    $inputNumbers = $selectionInput -split ',' | ForEach-Object { [int]$_.Trim() }
                    foreach ($num in $inputNumbers) {
                        if ($num -ge 1 -and $num -le $type2Recommendations.Count) {
                            $selectedType2Indices += ($num - 1)
                        }
                    }
                }
                catch {
    ...
    # Non-Interactive Mode: Run all Type1 audits, then all Type2 modules
    elseif ($NonInteractive) {
        ...
        # Build diff lists and select actionable Type2 tasks
        $diffResultSet = $null
        if (Get-Command -Name 'New-DiffListSet' -ErrorAction SilentlyContinue) {
            $diffResultSet = New-DiffListSet -MainConfig $MainConfig -AuditResults $AuditResults -OSContext $script:OSContext
        }
        ...
        $diffLists = if ($diffResultSet -and $diffResultSet.DiffLists) { $diffResultSet.DiffLists } else { @{} }
        $script:DiffLists = $diffLists
        $ExecutionParams.SelectedTasks = @()

        foreach ($task in $AvailableTasks) {
            $moduleName = if ($task.ModuleName) { $task.ModuleName } else { $task.Name }
            if ($diffLists.ContainsKey($moduleName) -and $diffLists[$moduleName].DiffCount -gt 0) {
                $ExecutionParams.SelectedTasks += $task
            }
            else {
                Write-Information "  [SKIP] $moduleName - no actions to be taken" -InformationAction Continue
            }
        }

        Write-Information "  Selected $($ExecutionParams.SelectedTasks.Count) tasks for execution" -InformationAction Continue
    }
    ...
    if ($TaskNumbers) {
        ...
        if (Test-Path $type1Path) {
            ...
            foreach ($file in $type1Files) {
                try {
                    ...
                    Write-Warning "Failed to run Type1 module $($file.BaseName): $($_.Exception.Message)"
                }
            }
        }
        ...
        if ($diffResultSet -and $diffResultSet.DiffLists) {
            $script:DiffLists = $diffResultSet.DiffLists
        }
        ...
        try {
            ...
            foreach ($taskNum in $taskNumbersArray) {
                if ($taskNum -ge 1 -and $taskNum -le $AvailableTasks.Count) {
                    $candidateTask = $AvailableTasks[$taskNum - 1]
                    $moduleName = if ($candidateTask.ModuleName) { $candidateTask.ModuleName } else { $candidateTask.Name }
                    if ($diffLists.ContainsKey($moduleName) -and $diffLists[$moduleName].DiffCount -gt 0) {
                        $ExecutionParams.SelectedTasks += $candidateTask
                    }
                    else {
                        Write-Information "  [SKIP] $moduleName - no actions to be taken" -InformationAction Continue
                    }
    ...
    # Skip task execution if Audit-Only mode already completed (no maintenance tasks needed)
    if ($ExecutionParams.Mode -eq 'AuditOnlyComplete') {
        Write-Information "`n=== Skipping task execution - system already optimized ===" -InformationAction Continue
        ...
        try {
            ...
            if (Test-Path $type1InventoryPath) {
                ...
                if ($systemInventory) {
                    Write-Information "  [OK] System inventory completed" -InformationAction Continue
                    ...
                    if ($script:ResultCollectionEnabled) {
                        ...
                        Write-Information "  [OK] Inventory result collected for reporting" -InformationAction Continue
                    }
                }
                else {
        ...
        <#
        .SYNOPSIS
            Extracts a numeric count from audit data.

        .DESCRIPTION
            Walks nested property paths on audit data to return a usable count,
            defaulting to zero when values are missing.

        .PARAMETER Data
            Audit data object to inspect.

        .PARAMETER PropertyPaths
            Ordered list of property paths to attempt.

        .OUTPUTS
            [int] Extracted count value.
        #>
        ...
        #region Stage 4: EXECUTING MAINTENANCE ACTIONS
        Write-Information "" -InformationAction Continue
        Write-Information "" -InformationAction Continue
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
        Write-Host "║                                                               ║" -ForegroundColor Cyan
        Write-Host "║           STAGE 4: EXECUTING MAINTENANCE ACTIONS              ║" -ForegroundColor Yellow
        Write-Host "║                                                               ║" -ForegroundColor Cyan
        Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
        Write-Information "" -InformationAction Continue
        Write-Host "  Executing $($ExecutionParams.SelectedTasks.Count) selected task(s)..." -ForegroundColor Green
        Write-Information "" -InformationAction Continue
        #endregion

        # Log execution start
        Write-LogEntry -Level 'INFO' -Component 'ORCHESTRATOR' -Message "Stage 4: Starting maintenance execution" -Data @{
            ExecutionMode = $executionMode
        ...
        for ($i = 0; $i -lt $ExecutionParams.SelectedTasks.Count; $i++) {
            ...
            try {
                ...
                try {
                    ...
                    $command = Get-Command -Name $task.Function -ErrorAction Stop
                    $moduleKey = if ($task.ModuleName) { $task.ModuleName } else { $task.Name }
                    $diffList = if ($script:DiffLists -and $script:DiffLists.ContainsKey($moduleKey)) { $script:DiffLists[$moduleKey].DiffList } else { $null }

                    $invokeParams = @{ Config = $MainConfig }
                    if ($diffList -and $command.Parameters.ContainsKey('DiffList')) {
                        $invokeParams.DiffList = $diffList
                    }

                    $result = & $task.Function @invokeParams
                ...
                if ($_.InvocationInfo.ScriptLineNumber) {
                    Write-Information "  Info: Error at line: $($_.InvocationInfo.ScriptLineNumber)" -InformationAction Continue
                }
                if ($_.ScriptStackTrace) {
...
if ($finalReports.Count -gt 0) {
    Write-Information "" -InformationAction Continue
    Write-Information " =======================================================" -InformationAction Continue
    Write-Host " Final Reports Available (Safe Location):" -ForegroundColor Green
    Write-Information " =======================================================" -InformationAction Continue
    Write-Host "   Location: $reportCopyTarget" -ForegroundColor Cyan
    ...
    Write-Information " =======================================================" -InformationAction Continue
    Write-Information "" -InformationAction Continue
    Write-Host "   These reports have been copied to the script.bat directory" -ForegroundColor Yellow
    Write-Host "   They will remain even if temporary files are cleaned up" -ForegroundColor Yellow
    Write-Information "" -InformationAction Continue
}
else {
...
#region Stage 5: REPORT GENERATION & CLEANUP
Write-Information "" -InformationAction Continue
Write-Information "" -InformationAction Continue
Write-Information "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan -InformationAction Continue
Write-Information "║                                                               ║" -ForegroundColor Cyan -InformationAction Continue
Write-Information "║        STAGE 5: REPORT GENERATION & CLEANUP COUNTDOWN         ║" -ForegroundColor Yellow -InformationAction Continue
Write-Information "║                                                               ║" -ForegroundColor Cyan -InformationAction Continue
Write-Information "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan -InformationAction Continue
Write-Information "" -InformationAction Continue
Write-Information "  All maintenance operations complete!" -ForegroundColor Green -InformationAction Continue
Write-Information "  Reports have been generated and saved." -ForegroundColor Green -InformationAction Continue
Write-Information "" -InformationAction Continue

# v3.2 Post-Execution Shutdown Sequence (Enhanced Stage 5)
if (Get-Command -Name 'Start-MaintenanceCountdown' -ErrorAction SilentlyContinue) {
    try {
        ...
        $shutdownConfig = @{
            ...
            RebootOnlyForWindowsUpdates = $false
        }

        Write-Information "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Yellow -InformationAction Continue
        Write-Information "║                                                               ║" -ForegroundColor Yellow -InformationAction Continue
        Write-Information "║                 CLEANUP COUNTDOWN STARTING                    ║" -ForegroundColor Red -InformationAction Continue
        Write-Information "║                                                               ║" -ForegroundColor Yellow -InformationAction Continue
        Write-Information "║  Press ANY KEY to abort and leave everything as-is          ║" -ForegroundColor Yellow -InformationAction Continue
        Write-Information "║  Or wait $($shutdownConfig.CountdownSeconds) seconds to clean and restart automatically   ║" -ForegroundColor Yellow -InformationAction Continue
        Write-Information "║                                                               ║" -ForegroundColor Yellow -InformationAction Continue
        Write-Information "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Yellow -InformationAction Continue
        Write-Information "" -InformationAction Continue

        Write-LogEntry -Level 'INFO' -Component 'ORCHESTRATOR' -Message "Stage 5: Initiating shutdown sequence" -Data $shutdownConfig
        ...
        Write-Information "" -InformationAction Continue
        Write-Information "  Shutdown action: $($shutdownResult.Action)" -ForegroundColor Cyan -InformationAction Continue
    }
    catch {
        Write-LogEntry -Level 'ERROR' -Component 'ORCHESTRATOR' -Message "Stage 5: Shutdown sequence failed: $($_.Exception.Message)"
        Write-Warning "  Shutdown sequence error: $_"
    }
}
else {
    Write-LogEntry -Level 'WARNING' -Component 'ORCHESTRATOR' -Message "Stage 5: ShutdownManager module not available - skipping countdown"
    Write-Warning "  ShutdownManager not available - skipping post-execution countdown"
}
#endregion
