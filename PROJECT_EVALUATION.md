...
| H6 | ~~Repo extraction relocated to `%ProgramData%\WindowsMaintenance`~~ **REVERTED** | script.bat ~168-171, ~397-430 | Implemented, then explicitly reverted per user request: the repo is downloaded/extracted next to `script.bat` again (`ZIP_FILE=%WORKING_DIR%update.zip`, `EXTRACTED_PATH=%WORKING_DIR%%EXTRACT_FOLDER%`), same as the original design. The OneDrive-sync / Stage-5-delete-lock concern from G item 2 is therefore still live — mitigated only by the `Set-Location` fix in H9, not by relocating the extraction target. |
...
**H10 (2026-07-18, later same day) — winget-under-elevation hardening + diagnostic fix.** Production log showed every `winget` operation in a Stage 3 run (`source update`, 5 removes, 9 installs, 2 upgrades) failing with the identical exit code `-1978335174` (`0x8A15003A`), starting with the very first call. Reproduced the exact `Invoke-ExternalPackageCommand` invocation mechanism directly on this machine non-elevated — it succeeded cleanly, isolating the failure to something specific to the orchestrator's always-elevated execution context. Leading, evidence-backed (not proof-certain — could not force a real UAC-elevated repro from this session) hypothesis: `winget` resolves via PATH to the per-user WindowsApps **App Execution Alias** stub, whose packaged-app activation is known to be unreliable when invoked from an elevated process. Fixed in `modules/core/Maintenance.psm1`:
- `Invoke-ExternalPackageCommand` previously **discarded** captured stdout/stderr, returning only the bare exit code — so no log ever showed *why* an external command failed. It now logs up to 600 chars of the real output on any non-zero exit.
- Added `Resolve-WingetPath` (exported): validates candidates by actually running `--version`, preferring the real machine-wide `Program Files\WindowsApps\...\winget.exe` over the per-user alias, falling back to the alias then to bare `'winget'` — cached per session. Can never regress today's behavior since it only switches to a path it has proven runs.
- Wired into every winget call site: `SoftwareManagement.psm1` (source update, install, upgrade, layer-4 uninstall), `SoftwareManagementAudit.psm1` (WinGet detection source, essential-app check), `SystemConfiguration.psm1` (Sysmon install), `Get-WingetUpgrade` in the core module.

**H11 (same day) — stale console input caused Stage 5's abort-countdown to self-trigger.** User-reported symptom: "Reboot ABORTED by user" firing with no keypress. Root cause: `[Console]::KeyAvailable`/`ReadKey` read from the console's persistent input buffer, which accumulates ANY keystroke typed into that window for the entire process lifetime — including stray/accidental keys pressed during the (often long, unattended) Stage 1-4 execution, hours before Stage 5's countdown ever starts polling. The first `KeyAvailable` check of the loop then sees that stale keystroke and treats it as a deliberate abort. Added `Clear-PendingConsoleInput` (drains any already-buffered keys via a `while ([Console]::KeyAvailable) { ReadKey }` loop) and call it immediately before starting **both** timed input loops — Stage 1's `Get-MenuSelection` and Stage 5's reboot countdown — so only keys pressed *during* the visible countdown can trigger a selection/abort.

---

# I. LOGGING & REPORT REDESIGN (v6, 2026-07-18)

Full rewrite of `modules/core/ReportGenerator.psm1` (859 → ~900 lines) plus a new log parser. Requested: reevaluate the logging mechanism/parsing, a more beautiful full-width report, PC info on top.

**Logging mechanism assessment.** The *writer* side (`Write-Log`/`Initialize-LogFile` in `Maintenance.psm1`) is mechanically sound and was left unchanged: direct auto-flushed `StreamWriter`, `FileShare.ReadWrite` (so the report can read the log live), per-sink level gating, stable single-line `[ts] [LEVEL] [COMPONENT] message` format. The actual weakness was entirely on the *consumption* side — the old report just `HtmlEncode`d the whole raw log into one opaque `<pre>` at the bottom, with zero structure. That single-line format is, however, exactly what makes the log cleanly machine-parseable, so the fix was to parse it rather than change how it's written.

**What changed (report generator):**
- **`ConvertFrom-MaintenanceLog`** (new, exported) — parses the log into structured entries (`Ts`/`ShortTs`/`Level`/`Component`/`Message`) via one regex; non-matching lines (launcher banners, separators) are preserved as `Level='RAW'` so nothing is dropped. Reads through a `FileStream` with `FileShare.ReadWrite` so it works while the core logger still holds the file open (Stage 4 live embedding).
- **Interactive log console** (`Build-LogConsole`) replaces the `<pre>` dump: per-level counts, a stacked level-distribution bar, clickable level-filter chips (DEBUG + RAW default-off to cut noise), a component dropdown, and a live text-search box. All filtering is self-contained vanilla JS (`Get-ReportJs`) — no `$`/backtick/template-literal so it survives the PowerShell here-string; no external dependencies.
- **PC "System Overview" at the very top** (`Build-SystemOverview`) — identity (host/user/domain/admin pill), OS (edition, build, install date, computed uptime), CPU (cores/threads/clock), memory, per-drive storage **meters** (green/amber/red by usage), and network — pulled from `system-inventory.json`, degrading gracefully to `OSContext`/env when the inventory module didn't run. A quick-facts strip carries PowerShell version, run-as, installed-app count, restore-point count, external IP.
- **Full-viewport width** — `.wrap { max-width:100% }` (was `1200px`); every grid uses `repeat(auto-fit, minmax(...))` so content reflows edge-to-edge on wide monitors instead of a narrow centered column.
- **Refined visual system** — CSS custom-property theming with a **light/dark toggle** (persisted to `localStorage`), gradient hero, restyled stat tiles + module cards (hover lift, metric grid), and a new "Disk reclaimed (MB)" run-stat aggregated from `DiskCleanup` ExtraData. Existing inventory/restore/health sections were restyled and de-duplicated (network moved into the top overview; users + restore points became a compact two-column detail grid).
...
**Fix v1 (guard — superseded):** first attempt added a `restart_flag.tmp` loop-guard in the stable launch folder enforcing at-most-one relaunch. In practice this proved insufficient — a run 10 minutes after the guard was committed still looped many times (GitHub's zipball CDN caches the downloaded `master.zip` for minutes after a push, and the guard is fragile inside deeply-nested batch blocks).

**Fix v2 (relaunch removed entirely — current):** the self-relaunch was **deleted**. It was never necessary: `:FIND_PWSH` resolves pwsh by **absolute install path** (Program Files\PowerShell, WindowsApps alias, Chocolatey, MSIX InstallLocation), not by `PATH`, so relaunching for a "fresh environment" could never help. Now, if an install reports success but pwsh still isn't located, the launcher **fails cleanly once** with manual-install instructions and `EXIT /B 1` — the infinite-window loop is impossible by construction, regardless of CDN caching or batch-parsing quirks. Removed the guard scaffolding (`PS7_ALREADY_RELAUNCHED`, the flag write, the `START cmd` relaunch); `:AFTER_RESTART_CHECK` now just sweeps away any legacy `restart_flag.tmp`. The "config missing / no orchestrator" symptoms were downstream damage from concurrent looping instances fighting over the extract folder, so removing the loop resolves those too.

**Two things this does NOT fix (require separate action):**
1. **The tool still can't RUN until PowerShell 7 is actually present.** The loop was masking a real bootstrap failure (PS7 not installing/detecting on the affected machines). The new code surfaces it as a clean one-shot error instead of a loop — the operator must install PS7 manually once (`winget install --id Microsoft.PowerShell --source winget --exact`, after ensuring winget/`EnableAppInstaller` is enabled).
2. **Self-update / CDN latency.** script.bat re-downloads from GitHub `master` every run and runs the *downloaded* copy, and GitHub's zipball is CDN-cached for minutes after a push — so a fix isn't live the instant it's pushed. Validate locally without the launcher via `pwsh -File .\MaintenanceOrchestrator.ps1` (elevated) against the working tree.
